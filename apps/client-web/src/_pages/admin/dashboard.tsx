...
export default function AdminDashboard() {
    ...
    const load = async () => {
      ...
      try {
        const [metricsRes, docsRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/admin/documents/pending`).then((r) =>
            r.json(),
          ),
        ]);
        if (!active) return;
        if (metricsRes.ok === false)
          throw new Error(
            metricsRes.error || "No se pudieron cargar las metricas",
          );
        if (docsRes.ok === false)
          throw new Error(
            docsRes.error || "No se pudo cargar la documentacion",
          );
  ...
  const navItems = useMemo(
    () => [
      { href: "/admin/dashboard", label: "KPI generales" },
      {
        ...
        label: "Revision de docs",
        badge: pendingDocs.length ? String(pendingDocs.length) : undefined,
      },
    ...
    const items = [
      {
        ...
        value: metrics.providersVerified,
        helper: `${metrics.providersTotal} activos`,
      },
      {
        title: "Urgencias en curso",
  ...
  const alerts = useMemo(() => {
    ...
    if (metrics.urgentActive > 3) {
      list.push({
        id: "alert-urgent",
        message: "Hay urgencias activas. Verificar SLA.",
        severity: "critico",
      });
    }
    if (metrics.documentsPending > 0) {
      list.push({
        id: "alert-docs",
        message: `${metrics.documentsPending} documentos requieren revision.`,
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {pendingDocs.map((item) => (
                    <tr
                      key={`${item.providerId}-${item.type || "doc"}`}
