...
import ProviderAvailability from "../../_components/ProviderAvailability";
import ProviderNotifications from "../../_components/ProviderNotifications";
...
export default function ProviderDashboard() {
    ...
    const load = async () => {
      try {
        const [
          ...
          apiRequest("/api/providers/" + providerId + "/materials").catch(
            () => ({ materials: [] }),
          ),
        ]);
  ...
  const handleCreate = async (event) => {
    ...
    try {
      if (!payload.catalogId)
        throw new Error("Selecciona un rubro del catalogo.");
      if (!payload.price || Number.isNaN(payload.price))
        throw new Error("Indica un precio valido.");
  ...
  const refreshMaterials = async () => {
    ...
    const res = await apiRequest(
      "/api/providers/" + providerId + "/materials",
    ).catch(() => ({ materials: [] }));
  ...
  const handleMaterialSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        API_BASE + "/api/providers/" + providerId + "/materials",
        {
          method: "POST",
          body: formData,
        },
  ...
  const handleCancellationSubmit = async (event) => {
    ...
    try {
      ...
      if (
        ...
        cancellationForm.proposedDate &&
        cancellationForm.proposedStart &&
        cancellationForm.proposedEnd
      ) {
      ...
      await apiRequest(
        "/api/requests/" + cancellationForm.requestId + "/cancel",
        {
          method: "POST",
          body: JSON.stringify(payload),
        },
  ...
  const renderDocuments = () => {
    ...
    return documents.map((doc) => {
      const docUrl =
        doc.url && (doc.url.startsWith("http") ? doc.url : API_BASE + doc.url);
      return (
        <div
          key={doc.type}
          style={{
        ...
        >
          ...
          <span style={{ color: statusColors[doc.status] || "#555" }}>
            {doc.status}
          </span>
          {docUrl ? (
          ...
          {doc.notes ? (
            <small style={{ color: "#555" }}>{doc.notes}</small>
          ) : null}
        </div>
      );
    });
    ...
    return services.map((srv) => {
      ...
      return (
        ...
        >
          ...
          >
            <div>
              ...
              <div style={{ color: "#555", fontSize: 13 }}>
                {srv.catalog?.rubro} � {srv.catalog?.subrubro}
              </div>
              <div style={{ color: "#777", fontSize: 13 }}>
                Categoria: {srv.category} � {srv.subCategory}
              </div>
            </div>
          ...
          {srv.notes ? (
            <p style={{ margin: 0, color: "#555" }}>{srv.notes}</p>
          ) : null}
    ...
    return materials
      ...
      .map((material) => {
        ...
        const attachmentUrl =
          ...
          (material.attachmentUrl.startsWith("http")
            ? material.attachmentUrl
            : API_BASE + material.attachmentUrl);
        return (
          <div
            key={material.id}
            style={{
          ...
          >
            ...
            <div style={{ color: "#555", fontSize: 13 }}>
              Solicitud: {material.requestId}
            </div>
            <div style={{ color: "#555", fontSize: 13 }}>
              Importe: {amount} {material.currency}
            </div>
            <div style={{ color: "#777", fontSize: 12 }}>
              Registrado: {new Date(material.createdAt).toLocaleString("es-AR")}
            </div>
            {attachmentUrl ? (
              <a
                href={attachmentUrl}
      ...
      <RequireRole role="PROVIDER">
        ...
        >
            ...
            >
              <div>
                ...
                <p style={{ margin: 0, color: "#555" }}>
                  Gestiona tus servicios, agenda, materiales y notificaciones.
                </p>
              </div>
              {user?.name ? (
                <span style={{ color: "#555" }}>Hola, {user.name}</span>
              ) : null}
            </div>
          ...
          >
            ...
            <p style={{ color: "#555" }}>
              Selecciona los barrios donde queres operar. Los clientes veran tu
              perfil solo en esas zonas.
            </p>
              ...
              {barrios.map((barrio) => (
                <label
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    gap: 8,
                    background: areasDraft.includes(barrio)
                      ? "var(--accent-soft)"
                      : "#f9fafb",
                    border: "1px solid var(--border)",
                    borderRadius: 12,
                ...
                >
                  <input
                    type="checkbox"
                    checked={areasDraft.includes(barrio)}
                    onChange={() => toggleArea(barrio)}
                  />
                  {barrio}
                </label>
              ))}
            ...
            >
              ...
              {docMessage && (
                <span style={{ color: "#16a34a" }}>{docMessage}</span>
              )}
          ...
          >
            ...
            <p style={{ color: "#555" }}>
              Los documentos enviados se revisan antes de habilitar tus
              servicios. Subi archivos claros y legibles para acelerar la
              aprobacion.
            </p>
          ...
          >
            <div>
              ...
              <p style={{ color: "#555" }}>
                Selecciona servicios del catalogo maestro, ajusta tu tarifa y
                mantene tu informacion al dia. Los badges indican si el servicio
                admite urgencias o distintas modalidades.
              </p>
            </div>
            ...
            >
              ...
              {selectedCatalog && (
                ...
                >
                  <span>
                    <strong>Categoria:</strong> {selectedCatalog.categoria} �{" "}
                    {selectedCatalog.subcategoria}
                  </span>
                  <span>
                    <strong>Modalidades:</strong>{" "}
                    {(selectedCatalog.modalidades || []).join(", ") || "N/A"}
                  </span>
                  <span>
                    <strong>Urgencias:</strong>{" "}
                    {selectedCatalog.permiteUrgencias ? "Si" : "No"}
                  </span>
                  {selectedCatalog.tiempoEstimado ? (
                    <span>
                      <strong>Tiempo estimado:</strong>{" "}
                      {selectedCatalog.tiempoEstimado} minutos
                    </span>
                  ) : null}
                  {selectedCatalog.etiquetas?.length ? (
                    <span>
                      <strong>Etiquetas:</strong>{" "}
                      {selectedCatalog.etiquetas.join(", ")}
                    </span>
                  ) : null}
                </div>
              )}
          ...
          >
            ...
            <p style={{ color: "#555" }}>
              Digitaliza los materiales utilizados para cada solicitud. Puedes
              adjuntar facturas o fotos legibles.
            </p>
            <form
            ...
            >
              ...
              {materialMessage && (
                <span style={{ color: "#16a34a" }}>{materialMessage}</span>
              )}
              {materialError && (
                <span style={{ color: "#c62828" }}>{materialError}</span>
              )}
            </form>
          ...
          >
            ...
            <p style={{ color: "#555" }}>
              Este formulario notifica al cliente cuando no podes asistir.
              Recorda indicar un motivo claro. Si propones otro horario, se
              mantiene la retencion operativa del 50% hasta que el cliente
              confirme.
            </p>
            <form
            ...
            >
                ...
                >
                  <label>
                    ...
                    <input
                      type="date"
                      name="proposedDate"
                      value={cancellationForm.proposedDate}
                      onChange={handleCancellationField}
                      required
                    />
                  </label>
                  <label>
                    ...
                    <input
                      type="time"
                      name="proposedStart"
                      value={cancellationForm.proposedStart}
                      onChange={handleCancellationField}
                      required
                    />
                  </label>
                  <label>
                    ...
                    <input
                      type="time"
                      name="proposedEnd"
                      value={cancellationForm.proposedEnd}
                      onChange={handleCancellationField}
                      required
                    />
                  </label>
              ...
              {cancellationMessage && (
                <span style={{ color: "#16a34a" }}>{cancellationMessage}</span>
              )}
              {cancellationError && (
                <span style={{ color: "#c62828" }}>{cancellationError}</span>
              )}
            </form>
          </section>
        </main>
