    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (active) {
          if (tplRes.ok === false)
            throw new Error(
              tplRes.error || "No se pudieron cargar los contratos",
            );
          if (insRes.ok === false)
            throw new Error(
              insRes.error || "No se pudieron cargar los seguros",
            );
  ...
  return (
    <>
      ...
      >
        ...
        {loading && (
          <p style={{ color: "#555" }}>
            Cargando informacion de contratos y seguros...
          </p>
        )}
                  ...
                  {templates.map((tpl) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "12px" }}>
                        {new Date(tpl.lastUpdated).toLocaleDateString("es-AR")}
                      </td>
              ...
              {insurances.map((item) => (
                ...
                >
                  ...
                  <span style={{ color: "var(--text-soft)" }}>
                    Cobertura: {currency.format(item.coverage || 0)}
                  </span>
                  <span style={{ color: "var(--text-soft)" }}>
                  ...
                  {item.renewalDate && (
                    <span style={{ color: "#666", fontSize: 13 }}>
                      Renueva:{" "}
                      {new Date(item.renewalDate).toLocaleDateString("es-AR")}
                    </span>
                  )}
                  <span
                    style={{
