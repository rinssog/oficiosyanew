    ...
    return (
      <div style={boxStyle}>
        ...
        <p style={{ color: "#555", marginBottom: 8 }}>
          Versión {terms.version} · Actualizado el{" "}
          {new Date(terms.updatedAt).toLocaleString("es-AR")}
        </p>
        <div
          style={{
            whiteSpace: "pre-wrap",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {status ? (
            <p style={{ color: "#2e7d32", fontWeight: 600 }}>
              Aceptado el {new Date(status.createdAt).toLocaleString("es-AR")} ·
              Versión {status.version}
            </p>
          ) : (
            <p style={{ color: "#c62828" }}>
              Aún no aceptaste los términos vigentes.
            </p>
          )}
          <button
