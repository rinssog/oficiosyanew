...
export default function RequestServicePage() {
  ...
  const fetchSlots = useCallback(async () => {
    ...
    try {
      const res = await apiRequest(
        `/api/providers/${providerId}/slots?days=21`,
      );
      const items = res.slots || [];
      setSlots(items);
      const firstAvailable = items.find((slot) => slot.available > 0);
      setSelectedSlotId((prev) =>
        prev && items.some((slot) => slot.id === prev && slot.available > 0)
          ? prev
          : firstAvailable?.id || "",
      );
    } catch (err) {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/providers/${providerId}/public`,
        );
        const payload = await res.json();
        if (!res.ok || payload.ok === false)
          throw new Error(payload.error || "No se pudo obtener el servicio");
        const found = (payload.services || []).find(
          (srv) => srv.id === serviceId,
        );
        if (!found) throw new Error("Servicio no disponible.");
        setService(found);
        setProviderName(
          payload.owner?.name || payload.owner?.email || "Prestador",
        );
        await fetchSlots();
      } catch (err) {
  ...
  const priceLabel = useMemo(
    () => (service ? priceFmt.format((service.price || 0) / 100) : ""),
    [service],
  );

  const filteredSlots = useMemo(
    () => (showUrgentOnly ? slots.filter((slot) => slot.urgent) : slots),
    [slots, showUrgentOnly],
  );
    ...
    try {
      ...
      if (mode === "slot") {
        ...
        const start = new Date(
          `${manualSchedule.date}T${manualSchedule.startTime}`,
        );
        const end = new Date(
          `${manualSchedule.date}T${manualSchedule.endTime}`,
        );
        if (
      ...
      setMessage(
        "Solicitud enviada. El prestador confirmara horario y detalles.",
      );
      setNotes("");
      setIsUrgent(false);
      if (mode === "slot") {
      ...
      >
        ...
        {!loading && !error && service && (
          <>
            ...
            >
              <h1 style={{ margin: 0 }}>
                Solicitar: {service.catalog?.nombre || "Servicio"}
              </h1>
              <p style={{ margin: 0, color: "#555" }}>
                Prestador: {providerName}
              </p>
              <p style={{ margin: 0, color: "#555" }}>
                Precio pactado: {priceLabel}
              </p>
              <p style={{ margin: 0, color: "#666" }}>
                {service.notes || "Sin descripcion adicional."}
              </p>
            </section>
              ...
              >
                ...
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  <button
                    ...
                    style={{
                      ...
                      background: mode === "slot" ? "#2563eb" : "#fff",
                      color: mode === "slot" ? "#fff" : "#374151",
                      fontWeight: 600,
                      cursor: "pointer",
                    }}
                  >
                ...
                {mode === "slot" ? (
                  <div style={{ display: "grid", gap: 12 }}>
                    ...
                    {groupedSlots.length === 0 ? (
                      <p style={{ color: "#777" }}>
                        No hay turnos publicados en los proximos dias.
                      </p>
                    ) : (
                                ...
                                style={{
                                  ...
                                  background:
                                    selectedSlotId === slot.id
                                      ? "#eff6ff"
                                      : "#f9fafb",
                                  opacity: slot.available > 0 ? 1 : 0.6,
                                }}
                              >
                                <input
                                  type="radio"
                    ...
                    >
                      <label>
                        ...
                        <input
                          type="time"
                          name="startTime"
                          value={manualSchedule.startTime}
                          onChange={handleManualChange}
                          required
                          style={{
                            width: "100%",
                ...
                <label>
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="Incluye detalles del acceso, mascotas, urgencia, etc."
                    style={{
                      width: "100%",
                      padding: "10px 12px",
                      borderRadius: 10,
                ...
                >
                  ...
                  />{" "}
                  Marcar solicitud como urgencia
                </label>
                ...
                {!user && (
                  <p style={{ color: "#777", fontSize: 13 }}>
                    Inicia sesion para seguir el estado de tus solicitudes en tu
                    panel.
                  </p>
                )}
              </form>
